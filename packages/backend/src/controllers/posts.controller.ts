...
export class PostsController {
  ...
  @ApiOperation({
    summary: '게시글 목록 조회',
    description: '페이지네이션을 지원하는 게시글 목록을 조회합니다.',
  })
      ...
      paginatedPosts.data.map(async post => {
        ...
        if (post.type === 'match') {
          try {
            ...
            const isLiked = user
              ? await this.postService.checkUserLikeStatus(post.id, user.id)
              : false;
            const isHated = user
              ? await this.postService.checkUserHateStatus(post.id, user.id)
              : false;
  ...
  @ApiOperation({
    summary: '게시글 삭제',
    description: '게시글을 삭제합니다. 작성자 또는 관리자만 삭제 가능합니다.',
  })
