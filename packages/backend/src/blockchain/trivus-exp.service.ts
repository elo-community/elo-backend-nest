...
export class TrivusExpService {
  ...
  constructor(
    ...
    const normalizedSignerKey =
      trustedSignerKey && !trustedSignerKey.startsWith('0x')
        ? `0x${trustedSignerKey}`
        : trustedSignerKey;
    ...
    this.logger.log(
      `[DEBUG] CONTRACT_ADDRESS: ${this.configService.get<string>(`blockchain.contracts.trivusExp.${activeNetwork}`)}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `[DEBUG] Creating Wallet with private key length: ${normalizedSignerKey.length}`,
      );
      this.logger.log(
        `[DEBUG] Private key starts with: ${normalizedSignerKey.substring(0, 10)}...`,
      );
      this.trustedSigner = new ethers.Wallet(normalizedSignerKey, this.provider);
      this.logger.log(
        `[DEBUG] Wallet created successfully, address: ${this.trustedSigner.address}`,
      );

      this.contractAddress =
        this.configService.get<string>(`blockchain.contracts.trivusExp.${activeNetwork}`) || '';
  ...
  async getTrustedSigner(): Promise<string> {
    try {
      ...
      if (error.message && error.message.includes('network does not support ENS')) {
        this.logger.warn(
          'ENS not supported on this network, using configured trusted signer address',
        );
  ...
  async verifySignature(data: {
    ...
    try {
      ...
      if (deadline <= Math.floor(Date.now() / 1000)) {
        this.logger.warn(
          `[DEBUG] Signature expired: deadline ${deadline}, current ${Math.floor(Date.now() / 1000)}`,
        );
        return false;
      }
      ...
      this.logger.log(
        `[DEBUG] Address comparison: ${recoveredAddress.toLowerCase()} === ${trustedSigner.toLowerCase()} = ${isValid}`,
      );

      if (!isValid) {
        this.logger.warn(
          `[DEBUG] Signature verification failed: recovered address doesn't match trustedSigner`,
        );
  ...
  async getServiceStatus(): Promise<{
    ...
    const networkName = 'matic-amoy'; // Polygon Amoy Testnet
    ...
    try {
      ...
      this.logger.warn(
        `trustedSigner() read failed, falling back to backend signer address: ${(e as Error).message}`,
      );
    }
