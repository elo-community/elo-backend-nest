...
export class BlockchainService implements OnModuleInit {
  ...
  constructor(
    ...
    if (this.adminWalletConfig.privateKey) {
      this.adminWallets.set(
        'amoy',
        new ethers.Wallet(this.adminWalletConfig.privateKey, this.providers.get('amoy')),
      );
    }
  ...
  async onModuleInit() {
    ...
    this.distributors.set(
      'amoy',
      this.configService.get<string>('blockchain.contracts.distributor.amoy') || '',
    );
    // this.distributors.set('very', this.configService.get<string>('blockchain.contracts.distributor.very') || '');
    this.pools.set(
      'amoy',
      this.configService.get<string>('blockchain.contracts.rewardPool.amoy') || '',
    );
