...
export class PostLikeSystemService {
  ...
  private async initializeBlockchainConnection() {
    try {
      ...
      const trustedSignerPrivateKey = this.configService.get<string>(
        'blockchain.trustedSigner.privateKey',
      );
  ...
  async calculateAvailableTokens(postId: number, userAddress: string): Promise<number> {
    try {
      ...
      this.logger.error(
        `Failed to calculate available tokens for post ${postId}: ${error.message}`,
      );
  ...
  async createLikeSignature(
    ...
    try {
      ...
      this.logger.log(
        `User ${userAddress} has ${tokenInfo.availableTokens} EXP in available_token, but claiming ${amount} EXP from post ${postId}`,
      );
      ...
      this.logger.log(
        `Private key (first 10 chars): ${this.trustedSigner.privateKey.substring(0, 10)}...`,
      );
      ...
      this.logger.log(
        `Created like signature for post ${postId}, user ${userAddress}, nonce ${nonce}, amount: ${ethers.formatEther(amountInWei)} EXP`,
      );
