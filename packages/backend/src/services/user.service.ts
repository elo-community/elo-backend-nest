...
export class UserService {
  ...
  async updateAvailableTokens(walletAddress: string): Promise<User> {
    ...
    const availableAmount =
      await this.tokenAccumulationService.getTotalAccumulatedAmount(walletAddress);
    ...
    console.log(
      `[UserService] updateAvailableTokens: walletAddress=${walletAddress}, availableAmount=${availableAmount}, availableTokens=${availableTokens}`,
    );
   ...
   */
  async updateAvailableTokenDirectly(
    walletAddress: string,
    newAvailableToken: number,
  ): Promise<User> {
    const user = await this.findByWalletAddress(walletAddress);
    if (!user) {
      throw new NotFoundException('User not found');
  ...
  async syncTokenBalanceFromBlockchain(walletAddress: string): Promise<User> {
    ...
    try {
      ...
      if (balanceDifference !== 0) {
        ...
        console.log(
          `[UserService] Token balance synced for ${walletAddress}: ${currentBalance} → ${blockchainBalance} (diff: ${balanceDifference})`,
        );
      } else {
        console.log(
          `[UserService] Token balance already in sync for ${walletAddress}: ${blockchainBalance}`,
        );
      }
  ...
  async completeTutorialFirstPost(userId: number): Promise<{ user: User; tokenAccumulation: any }> {
    ...
    // 토큰 적립 생성
    const tokenAccumulation = await this.tokenAccumulationService.accumulateTutorialFirstPostReward(
      user.walletAddress,
    );
   ...
   */
  async completeTutorialFirstMatch(
    userId: number,
  ): Promise<{ user: User; tokenAccumulation: any }> {
    const user = await this.findById(userId);
    if (!user) {
      throw new NotFoundException('User not found');
    }
    ...
    const tokenAccumulation =
      await this.tokenAccumulationService.accumulateTutorialFirstMatchReward(user.walletAddress);
        ...
        if (user.walletAddress) {
          try {
            ...
            if (Math.abs(currentBalance - previousBalance) > 0.000001) {
              // 부동소수점 오차 고려
              await this.recordInitialTokenSync(
                user.id,
                user.walletAddress,
                previousBalance,
                currentBalance,
            ...
            console.log(
              `✅ Synced token amount for user ${user.id} (${user.walletAddress}): ${previousBalance} → ${currentBalance} EXP`,
            );
          } catch (error) {
            console.error(
              `❌ Failed to sync token amount for user ${user.id} (${user.walletAddress}): ${error.message}`,
            );
          }
        }
      }
   ...
   */
  private async recordInitialTokenSync(
    userId: number,
    walletAddress: string,
    ...
    try {
      ...
      console.log(
        `📝 Initial token sync recorded for user ${userId}: ${previousBalance} → ${currentBalance} EXP`,
      );
    } catch (error) {
