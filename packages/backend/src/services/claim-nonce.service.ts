...
export class ClaimNonceService {
  ...
  async getNextNonce(walletAddress: string): Promise<string> {
    try {
      ...
      this.logger.log(
        `Generated random nonce for ${walletAddress}: ${nonceHex} (count: ${nonceRecord.currentNonce.toString()})`,
      );
      return nonceHex;
    } catch (error) {
  ...
  async markNonceAsUsed(walletAddress: string, nonce: string): Promise<void> {
    try {
      ...
      if (nonceRecord) {
        ...
        await this.claimNonceRepository.save(nonceRecord);
        this.logger.log(
          `Marked nonce ${nonce} as used for ${walletAddress} (total used: ${nonceRecord.lastUsedNonce.toString()})`,
        );
      }
    } catch (error) {
