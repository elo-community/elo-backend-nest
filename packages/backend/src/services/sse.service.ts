...
export class SseService {
  ...
  sendMatchResultNotification(
    ...
    this.logger.log(
      `Sending MATCH_RESULT_REQUEST notification to user ${opponentUserId} for match ${matchResultId} in ${sportCategory}`,
    );
   ...
   * 경기 결과 상태 변경 알림 전송
   */
  sendMatchResultStatusNotification(
    userId: number,
    matchResultId: number,
    status: string,
    sportCategory: string,
    ...
    this.logger.log(
      `Sending MATCH_RESULT_STATUS_CHANGED notification to user ${userId} for match ${matchResultId} with status ${status} in ${sportCategory}`,
    );
