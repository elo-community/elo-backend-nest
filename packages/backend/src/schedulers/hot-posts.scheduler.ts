...
export class HotPostsScheduler {
  ...
  async selectHotPosts() {
    ...
    try {
      ...
      this.logger.log(
        `Calculating hot posts for period: ${yesterday.toISOString()} ~ ${today.toISOString()}`,
      );
      ...
      for (const post of allPosts) {
        ...
        try {
          ...
          this.logger.debug(
            `Post ${post.id}: Score=${popularityScore}, Likes=${likeCount}, Comments=${commentCount}, Hates=${hateCount}`,
          );
        } catch (error) {
      ...
      for (const post of topPosts) {
        this.logger.log(
          `- Rank ${topPosts.indexOf(post) + 1}: Post ID: ${post.id}, Title: "${post.title}", Score: ${post.popularityScore} (Likes: ${post.likeCount}, Comments: ${post.commentCount}, Hates: ${post.hateCount})`,
        );
      }
      ...
      this.logger.log(
        `Hot posts selection completed: ${savedHotPosts.length} posts selected, ${rewardSuccessCount} rewards distributed successfully, ${rewardFailureCount} rewards failed`,
      );
      this.logger.log(
        `Hot posts selection and storage completed successfully for ${today.toDateString()}`,
      );
    } catch (error) {
